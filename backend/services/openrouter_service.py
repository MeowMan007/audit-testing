"""
OpenRouter AI Insights Service — Calls OpenRouter LLM API for
deep accessibility analysis and design alignment insights.

Uses the free google/gemma-3-27b-it model via OpenRouter.
Provides natural-language analysis of audit results including:
...
class OpenRouterService:
    """Service for getting AI-powered accessibility insights via OpenRouter."""
    ...
    async def get_insights(
        ...
        """Get AI-powered insights from OpenRouter."""
        if not self.is_available:
            return {"available": False, "reason": "OPENROUTER_API_KEY not configured"}
        ...
        for attempt, current_model in enumerate(models_to_try):
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    response = await client.post(
                        ...
                        headers={
                            ...
                            "X-Title": "AccessLens Accessibility Audit",
                        },
                        json={
                            "model": current_model,
                            "messages": [
                    ...
                    if response.status_code == 429:
                        logger.warning(f"Rate limited (429) on model {current_model}. Attempt {attempt + 1}/{len(models_to_try)}")
                ...
                logger.info(f"OpenRouter insights generated for {url} using {current_model}")
                return parsed

            except httpx.HTTPStatusError as e:
                logger.error(f"OpenRouter API error on {current_model}: {e.response.status_code}")
                if attempt < len(models_to_try) - 1:
                    await asyncio.sleep(1.5)
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse OpenRouter JSON on {current_model}: {e}")
                if attempt < len(models_to_try) - 1:
                    await asyncio.sleep(1.5)
            except Exception as e:
                logger.error(f"OpenRouter request failed on {current_model}: {e}")
                if attempt < len(models_to_try) - 1:
                    await asyncio.sleep(1.5)
                    
        # If we exit the loop, all models failed (rate limits, bad json, etc.)
        logger.warning("All OpenRouter models failed. Falling back to mock insights.")
...
Provide 3-5 design_issues, exactly 3 top_fixes, and 3-4 ux_patterns. Be specific and actionable."""
    ...
    def _get_mock_insights(self, url: str, score: float, grade: str, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Return fallback mock insights when API rate limits are hit."""
        ...
        return {
            "available": True,
            "model_used": "mock-fallback-due-to-429",
            ...
            "ai_narrative": "Rate limits (429) prevented live AI analysis from OpenRouter, but these general best practices based on your audit results will guide you toward a more accessible design!"
        }
